...
Item {
    ...
    RectangularCursor {
        id: rectangleCursor
        x: canvas ? Math.floor(canvas.cursorPos.x + canvas.brushRect.x * canvas.currentPane.zoomLevel) : 0
        y: canvas ? Math.floor(canvas.cursorPos.y + canvas.brushRect.y * canvas.currentPane.zoomLevel) : 0
        z: 1
        width: canvas ? canvas.brushRect.width * canvas.currentPane.zoomLevel : 0
        height: canvas ? canvas.brushRect.height * canvas.currentPane.zoomLevel : 0

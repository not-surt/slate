...
{
    if (canvas->editingContextManager()->brushManager()->type() == brushType)
        return true;
...
bool TestHelper::selectColourAtCursorPos()
{
    if (tilesetProject) {
        ...
        const QColor actualColour = canvas->editingContextManager()->foregroundColour();
...
bool TestHelper::drawPixelAtCursorPos()
{
    if (tilesetProject) {
        ...
        expectedImage.setPixelColor(tileCanvas->scenePosToTilePixelPos(cursorPos), tileCanvas->editingContextManager()->foregroundColour());
        ...
        QColor expectedColour = canvas->editingContextManager()->foregroundColour();
...
bool TestHelper::addSwatchWithForegroundColour()
{
    ...
    const QString expectedDelegateObjectName = QString::fromLatin1("swatchGridView_Delegate_%1_%2")
        .arg(previousDelegateCount).arg(canvas->editingContextManager()->foregroundColour().name());
...
bool TestHelper::updateVariables(bool isNewProject, Project::Type projectType)
{
    ...
    canvas->editingContextManager()->setForegroundColour(Qt::black);
    canvas->editingContextManager()->setBackgroundColour(Qt::white);
